...
struct pardata {
    uint32_t*        start_at;
    uint32_t*        stop_at;
    std::unordered_map<uint32_t,uint32_t *>* pmaps;
    int            start_flag;
    uint32_t         otokens;
    pthread_t      tid;
...
#ifdef STATS
    uint32_t lookups, hits, values, zeros, entries, nulls, adjusts, pass_throughs, virgins;
#endif
};
...
static struct resbuf* newbuf ()
{
    ...
    results->buffer = (uint32_t *) malloc(OUTBUFSIZE*sizeof(uint32_t));
...
void* parscan (void* arg)
{
    ...
    uint32_t* m2buf = args->start_at;
    uint32_t* stop_at = args->stop_at;
    int start_flag = args->start_flag;
    std::unordered_map<uint32_t,uint32_t *>* pmaps = args->pmaps;
    uint32_t otokens = args->otokens;
    struct resbuf* resbuf, *uresbuf;
    int resolved_values, unresolved_values;
    uint32_t otoken;
#ifdef STATS
    uint32_t lookups = 0, hits = 0, values = 0, zeros = 0, entries = 0, adjusts = 0, pass_throughs = 0, virgins = 0;
#endif
	...
	while (*m2buf) {
	    ...
	    lookups++;
#endif
	    std::unordered_map<uint32_t,uint32_t*>::const_iterator maps_iter = pmaps->find(*m2buf);
		...
		if (maps_iter->second) {
		    ...
		    resolved_values = 0;
		    unresolved_values = 0;
		    for (uint32_t* mbuf = maps_iter->second; *mbuf; mbuf++) {
#ifdef DEBUG_TARGET
			if (DEBUG_TARGET(otoken)) {
...
int main(int argc, char** argv)
{
    ...
    int finish_flag = 0;
    int mfd, m2fd;
    u_long mdatasize, mmapsize, m2datasize, ma2datasize = 0, m2mapsize;
    uint32_t* mbuf, *m2buf, *morig;
    long rc;
    uint32_t addr, tokens, tokens2, otoken, otokens;
    std::unordered_map<uint32_t,uint32_t *> maps;
    std::unordered_set<uint32_t> new_maps;
#ifdef STATS
    char statsfile[256];
    ...
    sprintf (outputsufile, "%s/%s/merge-outputs-unresolved", dirname, me1);
    sprintf (outputsrfile, "%s/%s/merge-outputs-resolved", dirname, me1);
    sprintf (addrsfile, "%s/%s/%s-addrs", dirname, me1, mof);
    sprintf (mergefileo2, "%s/%s/merge-outputs-unresolved", dirname, me2);
    sprintf (mergefilea1, "%s/%s/%s-addrs", dirname, me1, mf1);
    sprintf (mergefilea2, "%s/%s/%s-addrs", dirname, me2, mf2);
...
#ifdef DEBUG_TARGET
    char debugname[256];
    sprintf (debugname, "%s/%s/%s-debug", dirname, me1, mof);
    ...
    if (parallelize > 1 && m2datasize > 0) {
        uint32_t chunk = m2datasize/(parallelize*sizeof(uint32_t));
	uint32_t* start_at = m2buf;
	...
	for (int i = 0; i < parallelize; i++) {
	    ...
	    if (i == parallelize-1) {
		pdata[i].stop_at = (uint32_t *) ((u_long) m2buf + m2datasize);
	    } else {
		start_at += chunk;
	...
	for (int i = 0; i < parallelize; i++) {
		...
		rc = write (outrfd, results->buffer, results->bufindex*sizeof(uint32_t));
		if (rc != (long)  (results->bufindex*sizeof(uint32_t))) {
		    fprintf (stderr, "write of buffer segment failed, rc=%ld, epected %ld, errno=%d\n", 
			     rc, results->bufindex*sizeof(uint32_t), errno);
		    exit (rc);
		}
	    }
	    for (results = pdata[i].uresbuf; results; results = results->next) {
		rc = write (outufd, results->buffer, results->bufindex*sizeof(uint32_t));
		if (rc != (long)  (results->bufindex*sizeof(uint32_t))) {
		    fprintf (stderr, "write of buffer segment failed, rc=%ld, epected %ld, errno=%d\n", 
			     rc, results->bufindex*sizeof(uint32_t), errno);
		    exit (rc);
	...
	while ((u_long) m2buf < (u_long) morig + m2datasize) {
		...
		lookups++;
#endif
		std::unordered_map<uint32_t,uint32_t*>::const_iterator maps_iter = maps.find(*m2buf);
		if (maps_iter != maps.end()) {
			...
			if (DEBUG_TARGET(otoken)) {
			    fprintf (debugfile, "output %lx (otokens %lx/%lx) -> unresolved pass through %lx, offset %lx\n", 
				     otoken, otokens, otoken-otokens, *m2buf, (uint32_t) m2buf - (uint32_t) morig);
			}
#endif
			print_uvalue (0);
		    }
	...
	morig = m2buf;
	printf ("read output token %x\n", *m2buf);
	print_value (otokens + *m2buf); // Add otokens together
	printf ("write output token %x\n", otokens + *m2buf);
	m2buf++;
	printf ("read input token %x\n", *m2buf);
	tokens2 = *m2buf - 0xc0000000;
	printf ("write input token %x\n", tokens+tokens2);
...
#ifdef NO_DUPS
	    std::unordered_set<uint32_t>::const_iterator values_iter;
	...
	std::unordered_map<uint32_t,uint32_t*>::const_iterator maps_iter;
    ...
    sprintf (statsfile, "%s/%s/%s-stats", dirname, me1, mof);
