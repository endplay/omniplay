...
#endif
{
	...
	if (sysnum == 120) { 
	    printf("found a fork flags %x\n", (u_int)syscallarg0);
	    printf("%x",CLONE_CHILD_SETTID | CLONE_CHILD_CLEARTID | SIGCHLD);
	    if ((CLONE_CHILD_SETTID | CLONE_CHILD_CLEARTID | SIGCHLD) == (u_int)syscallarg0){ 
		printf("found an actual, bonafied fork\n");
	    }
	}
	if (sysnum == 31) {
...
//    PIN_AddThreadFiniFunction(thread_fini, 0); why do we need this...? 
