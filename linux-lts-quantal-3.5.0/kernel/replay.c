...
asmlinkage int shim_fork(struct pt_regs regs)
{
	SHIM_NOOP_HEAD(fork);
	printk ("Pid %d calls shim_fork\n", current->pid);
...
static asmlinkage long
long record_execve(const char __user *name,
		   ...
		   const char __user *const __user *envp, struct pt_regs *regs)
{
	...
	rc = sys_execve(name, argv, envp, regs);
...
static asmlinkage long
long replay_execve(const char __user *name,
		   const char __user *const __user *argv,
		   const char __user *const __user *envp, struct pt_regs *regs)
{
	struct replay_thread* prt = current->replay_thrd;
...
//SHIM_CALL(execve, 11, name, argv, envp, regs)
...
long sys_iopl(unsigned int level, struct pt_regs *regs); /* No prototype */
asmlinkage long 
shim_iopl(unsigned int level, struct pt_regs* regs) SHIM_NOOP(iopl, level, regs)

asmlinkage long shim_vhangup (void) SHIM_NOOP(vhangup)

int sys_vm86old(struct vm86_struct __user *v86, struct pt_regs* regs); /* No prototype */
asmlinkage int 
shim_vm86old(struct vm86_struct __user *v86, struct pt_regs* regs)
SHIM_NOOP(vm86old, v86, regs)
...
asmlinkage unsigned long shim_sigreturn(struct pt_regs regs)
{
	/* No need to log and replay, because this is deterministic.
	However, must execute actual syscall since it tears down the signal stack frame */
	if (current->record_thrd) {
		printk ("Pid %d does sigreturn\n", current->pid);
	}
	return sys_sigreturn(&regs);
}

static long record_clone(unsigned long clone_flags, unsigned long newsp, void __user *parent_tid, void __user *child_tid, struct pt_regs *regs)
{
	...
	rc = sys_clone(clone_flags, newsp, parent_tid, child_tid, regs);
...
static long replay_clone(unsigned long clone_flags, unsigned long newsp, void __user *parent_tid, void __user *child_tid, struct pt_regs *regs)
{
	struct pthread_log_head __user * phead = NULL;
	struct pthread_log_data __user * start, *old_start = NULL;
	...
	parent_tidptr = (int __user *) parent_tid;
	...
	if (rc > 0) {
		...
		pid = sys_clone(clone_flags, newsp, parent_tid, child_tid, regs);
...
asmlinkage long 
shim_clone(unsigned long clone_flags, unsigned long newsp, void __user *parent_tid, void __user *child_tid, struct pt_regs *regs) 
{	
	struct task_struct* tsk;
	int child_pid;

	if (current->record_thrd) return record_clone(clone_flags, newsp, parent_tid, child_tid, regs);		
	if (current->replay_thrd) {
		if (test_app_syscall(120)) return replay_clone(clone_flags, newsp, parent_tid, child_tid, regs);
		child_pid = sys_clone(clone_flags, newsp, parent_tid, child_tid, regs);
		//clear flag for child
	...
	return sys_clone(clone_flags, newsp, parent_tid, child_tid, regs);
}
...
int sys_vm86(struct pt_regs *regs); /* No prototype */
asmlinkage int shim_vm86(struct pt_regs regs)
{
	SHIM_NOOP_HEAD(vm86);
	return sys_vm86(&regs);
}
...
asmlinkage unsigned long shim_rt_sigreturn(struct pt_regs* regs)
{
	if (current->record_thrd) {
...
asmlinkage int shim_sigaltstack(unsigned long bx)
{
	/* Copy sys_sigaltstack() to preseve its strange GCC workarounds. */
	struct pt_regs *regs = (struct pt_regs *)&bx;
	const stack_t __user *uss = (const stack_t __user *)bx;
	stack_t __user *uoss = (stack_t __user *)regs->cx;
	int rc;

	// This should be deterministic - unless perhaps replaying with PIN?
	rc =  do_sigaltstack(uss, uoss, regs->sp);
	if (current->replay_thrd && current->replay_thrd->app_syscall_addr != 0) {
		MPRINT ("Pid %d sigaltstack replay with PIN - is this OK?  - uss %p oss %p\n", current->pid, uss, uoss);
	}
	return rc;
}
...
static int 
record_vfork (struct pt_regs *regs)
{
	long rc;
	...
	rc = do_fork(CLONE_VFORK | CLONE_VM | SIGCHLD, regs->sp, regs, 0, NULL, NULL);
...
static int 
replay_vfork (struct pt_regs *regs)
{
	struct task_struct* tsk = NULL;
	struct replay_thread* prt = current->replay_thrd;
	struct replay_group* prg = prt->rp_group;
	...
	if (rc > 0) {
		...
		pid = do_fork(CLONE_VFORK | CLONE_VM | SIGCHLD, regs->sp, regs, 0, NULL, NULL);
...
asmlinkage int shim_vfork(struct pt_regs regs) 
{
	if (current->record_thrd) return record_vfork(&regs);		
	if (current->replay_thrd) return replay_vfork(&regs);
...
struct getxattr_retvals {
	char value;	// Variable length buffer follows of size rc
};

asmlinkage ssize_t
record_getxattr (const char __user *path, const char __user *name,
		void __user *value, size_t size)
{
	long rc;
	struct getxattr_retvals *pretval = NULL;
	...
	if (rc > 0) {
		pretval = ARGSKMALLOC (sizeof(struct getxattr_retvals) + (rc-1), GFP_KERNEL);
		if (pretval == NULL) {
			printk ("record_getxattr: can't allocate buffer\n");
			return -ENOMEM;
		}
		if (copy_from_user(&pretval->value, value, rc)) {
			ARGSKFREE (pretval, sizeof(struct getxattr_retvals) + (rc-1));
			pretval = NULL;
			rc = -EFAULT;
		}
	}
...
asmlinkage ssize_t
replay_getxattr (const char __user *path, const char __user *name,
		void __user *value, size_t size)
{
	...
	if (retparams) {
		if (copy_to_user (value, &retparams->value, rc)) {
...
asmlinkage ssize_t
shim_getxattr (const char __user *path, const char __user *name,
	       void __user *value, size_t size)
//SHIM_NOOP(getxattr, path, name, value, size)
SHIM_CALL (getxattr, 229, path, name, value, size)

asmlinkage ssize_t
shim_lgetxattr (const char __user *path, const char __user *name, 
		void __user *value, size_t size)
SHIM_NOOP(lgetxattr, path, name, value, size)
		...
		if (psr[cnt].retparams) {
			...
			switch (psr[cnt].sysnum) {
				...
				case 220: size = psr[cnt].retval; break;
 			        case 221: size = sizeof(struct flock64); break;
				case 229: size = sizeof(struct getxattr_retvals) + ((psr[cnt].retval)-1); break;
				case 239: size = sizeof(struct sendfile64_retvals); break;
			        case 242: size = sizeof(cpumask_t); break;
				case 243: size = sizeof(struct set_thread_area_retvals); break;
