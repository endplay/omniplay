...
int track_usually_pt2pt_read(void *key, int size, struct file *filp) {
	...
	*is_cached = READ_IS_PIPE;
	...
	/* The pipe is not in the tree, this is its first write (by a recorded process) */
	if (info == NULL) {
		printk("WARNING: fifo read returns before write?!?!?!?\n");
		/* Create a new pipe_track */
		info = kmalloc(sizeof(struct pipe_track), GFP_KERNEL);
		/* Crap... no memory */
		if (info == NULL) {
...
int track_usually_pt2pt_write(void *key, int size, struct file *filp) {
	u64 rg_id = current->record_thrd->rp_group->rg_id;
	struct pipe_track *info;
	char *pretparams;
	/* Wohoo, we have a pipe.  Lets track its writer */
...
int consume_socket_args_read(void *retparams) {
	int consume_size = 0;
	u_int is_cache_file = *((u_int *)retparams);
...
int consume_socket_args_write(void *retparams) {
	int consume_size = 0;
...
extern const struct proto_ops unix_stram_ops;
...
record_socketcall(int call, unsigned long __user *args)
{
	...
	case SYS_SENDTO:
	{
		...
		if (rc > 0) {
			...
			struct sock *socket = filp->private_data;

			if (socket->ops == &unix_stream_ops || socket->ops == unix_seqpacket_ops) {
				int ret;
				struct sock *peer;
				peer = unix_per_get(socket);
				ret = track_usually_pt2pt_write(peer, a[1], rc);
				sock_put(peer);
				if (ret) {
					KARGSFREE(pretvals);
					return ret;
	...
	case SYS_RECV: 
	{
		...
		if (rc >= 0) {
			...
			do /* magic */ {
				...
				if (socket->ops == &unix_stream_ops || socket->ops == unix_seqpacket_ops) {
					int ret;
					ret = track_usually_pt2pt_read(socket, a[1], rc);
					if (ret) {
						KARGSFREE(pretvals);
						return ret;
					}
				} else {
					u_int *is_cached = KARGSMALLOC(sizeof(u_int), GFP_KERNEL);
					if (is_cached == NULL) {
						return -ENOMEM;
					}
					*is_cached = 0;
...
replay_socketcall (int call, unsigned long __user *args)
{
	...
	case SYS_SENDTO:
		if (retparams) {
			...
			ret = consume_socket_write_args(retparams);
		}
		return rc;
#else
	case SYS_SEND:
