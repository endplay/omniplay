...
int main (int argc, char* argv[]) 
{
	...
	if (fgets (line, 255, file)) {
	    struct epoch e;
	    rc = sscanf (line, "%d %c %u %c %u %u %u\n", &e.data.start_pid, &e.data.start_level, &e.data.start_clock, 
			 &e.data.stop_level, &e.data.stop_clock, &e.data.filter_syscall, &e.data.ckpt);
	    if (rc != 7) {
