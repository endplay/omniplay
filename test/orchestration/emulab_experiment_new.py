...
class Server:
    ...
    def start_stream_server(self, user, password):
        shell = experiment_utilities.open_ssh_session(self.host, user, password)
        with shell:
...
def main(): 
    ...
    parser.add_option("--seq_flags", dest="seq_flags")
    ...
    if options.sync:
        last_test.hosts[0].prep_for_ctrl("arquinn",password, last_test.partition_filename, last_test.server_config_file, last_test.correct_dir)
        if options.seq_flags != None and options.seq_flags != "":
            seq_flags = "-" + options.seq_flags
            last_test.start_ctrl("arquinn", password,[seq_flags,"-s","-stats", "-v", "tmp_results","seqtt.results"])
        else:
            last_test.start_ctrl("arquinn", password,["-seqppl","-s","-stats", "-v","tmp_results","seqtt.results"])
    ...
    for test in test_configurations:    
        ...
        for r in range(num_rounds):
            args = ["-stats"]
            if options.seq_flags != None and options.seq_flags != "":
                args.append("-" + options.seq_flags)
            else:
                args.append("-seqppl")
            
            if options.compress != None and options.compress: 
                args.append("-c")
            
            test.hosts[0].prep_for_ctrl("arquinn",password, last_test.partition_filename, last_test.server_config_file, None)
