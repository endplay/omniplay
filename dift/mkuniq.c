...
static long print_addrs (char* dirname)
{
    ...
    char* buf;
    u_long* p;
    u_long rc, num_addrs = g_hash_table_size(uniq_addrs), wsize;
    ...
    g_hash_table_iter_init(&iter, uniq_addrs);
    p = (u_long *) buf;
    ...
    sprintf (splice_name, "%s/uniq", dirname);
...
static long splice_before_segment (char* dirname)
{
    ...
    uniq_addrs = g_hash_table_new(NULL, NULL);
	...
	for (i = 0; i < buf_size; i++) {
	    ...
	    value = *((u_long *) plog);
	    plog += sizeof(u_long);
	    if (value && (value < 0xc0000001 || value > 0xe0000001)) {
		g_hash_table_add(uniq_addrs, GUINT_TO_POINTER(value));
	    }
	}
    }
