...
struct replay_group {
	...
	int rg_checkpoint_taken;    //we haven't checkpointed. 
...
new_replay_group (struct record_group* prec_group, int follow_splits)
{
	...
	prg->rg_checkpoint_taken = 0; //we haven't checkpointed. 
...
replay_full_ckpt (long rc)
{
	...
	struct pid_to_leader leader_list;
	struct pid_to_leader *tmp_leader;
	...
	printk("in replay_full_ckpt %s", ckpt);
	MPRINT ("replay_full_ckpt saving rc %ld\n", rc);


	INIT_LIST_HEAD(&leader_list.list);
	//we choose a leader pid for each of the thread groups in the replay. This leader is what
	iter = ds_list_iter_create(current->replay_thrd->rp_group->rg_replay_threads);
	while((tmp = ds_list_iter_next(iter)) != NULL) { 
		printk("about to allocate tmp_leader");
		tmp_leader = KMALLOC(sizeof(struct pid_to_leader), GFP_KERNEL);
		tmp_leader->pid = tmp->rp_replay_pid;
		tmp_leader->leader = -1; 
		/* add the new item 'tmp' to the list of items in mylist */
		printk("about to add tmp_leader");
		list_add(&(tmp_leader->list), &(leader_list.list));
		printk("added  tmp_leader for pid %d\n",tmp_leader->pid);
	}
	ds_list_iter_destroy(iter);
	...
	while ((tmp = ds_list_iter_next(iter)) != NULL) {	
		...
		found = 0;
		list_for_each_entry(tmp_leader, &(leader_list.list),list){
			if(tmp_leader->pid == tmp->rp_replay_pid) { 
				printk("found %d leader_list entry\n",tmp->rp_replay_pid);
				if (tmp_leader->leader > 0) { 
					printk("I guess %d already claimed %d\n",tmp_leader->leader,tmp->rp_replay_pid);
					found = 1;
					continue;
				}
				else { 
					printk("I'm my own leader! \n");
					tmp_leader->leader = tmp->rp_record_thread->rp_record_pid; //I'm my own leader!
				}
			}
		}
		if (!found){
			...
			if (tsk) {
				...
				while ((tmp2 = ds_list_iter_next(iter2)) != NULL) { // && tmp != tmp2) {	
					...
					if (tsk2 && tsk2->mm == tsk->mm) {
						printk ("We're %d's boss! \n",tmp2->rp_record_thread->rp_record_pid);
						list_for_each_entry(tmp_leader, &leader_list.list,list){
							if(tmp_leader->pid == tmp2->rp_replay_pid) { 
								tmp_leader->leader = tmp->rp_record_thread->rp_record_pid; //I'm my own leader!
							}
						}				     
					}
	...
	retval = replay_full_checkpoint_hdr_to_disk (ckpt, precg->rg_id, clock, proc_count, &pos);
	if (retval) return retval;

	// First write out data for this process
	printk ("Checkpointing task %d status %d exp clock %lu\n", current->pid, current->replay_thrd->rp_status, current->replay_thrd->rp_expected_clock);

	list_for_each_entry(tmp_leader, &leader_list.list,list){
		if(tmp_leader->pid == current->replay_thrd->rp_replay_pid) { 
			cleader = tmp_leader->leader; //grab our leader
		}
	}				     
	retval = replay_full_checkpoint_proc_to_disk (ckpt, current, current->replay_thrd->rp_record_thread->rp_record_pid, cleader,rc, 
						      current->replay_thrd->rp_record_thread->rp_read_log_pos, current->replay_thrd->rp_out_ptr, 
						      argsconsumed(current->replay_thrd->rp_record_thread), current->replay_thrd->rp_expected_clock, &pos);
	if (retval) return retval;
	...
	while ((tmp = ds_list_iter_next(iter)) != NULL) {	
		...
		if (tsk && tsk != current) {
			...
			list_for_each_entry(tmp_leader, &leader_list.list,list){
				if(tmp_leader->pid == prt->rp_replay_pid) { 
					cleader = tmp_leader->leader; //grab our leader
				}
			}				     

			printk ("Checkpointing task %d status %d exp clock %ld\n", tsk->pid, prt->rp_status, prt->rp_ckpt_save_expected_clock);
			retval = replay_full_checkpoint_proc_to_disk (ckpt, tsk, prt->rp_record_thread->rp_record_pid, cleader, 0,
	...
	list_for_each_safe(p, q, &(leader_list.list)){
		tmp_leader = list_entry(p, struct pid_to_leader, list);
		list_del(p);
		kfree(tmp_leader);
	}
			 ...
			 int follow_splits, int save_mmap, loff_t attach_index, int attach_pid)
{
	...
	record_pid = replay_full_resume_proc_from_disk(ckpt, current->pid, &retval, &prect->rp_read_log_pos, &prept->rp_out_ptr, &consumed, &prept->rp_expected_clock, &pos);
	DPRINT ("Pid %d gets record_pid %ld exp clock %lu\n", current->pid, record_pid, prept->rp_expected_clock);
	if (record_pid < 0) {
...
long
replay_full_ckpt_proc_wakeup (char* logdir, char* filename, int fd)
{
	...
	mutex_lock(&ckpt_mutex); 
	record_pid = replay_full_resume_proc_from_disk (pckpt_waiter->ckpt, pckpt_waiter->clock_pid, &retval, &prect->rp_read_log_pos, 
							&prept->rp_out_ptr, &consumed, &prept->rp_expected_clock, &pckpt_waiter->pos);
	...
	MPRINT ("Pid %d replay_full_ckpt_proc_wakeup restarting syscall %d\n", current->pid, prect->rp_log[prept->rp_out_ptr].sysnum);
