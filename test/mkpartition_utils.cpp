...
unordered_set<short> bad_syscalls({120, 174, 125, 45}); //256 maybe doens't need to be in here? I had some issues with the longerfirefox so... whatever
...
static int map_next_file(char * filename, u_long* &log, u_long &data_size, u_long &mapsize, int &fd) { 
    ...
    fprintf(stderr,"%s endat %lu mapsize %lu\n", filename, data_size, mapsize);
...
int parse_pin_traces_saved(vector<struct timing_data> &td, char* dir, u_int epochs) { 
    ...
    u_long num_saved = 0;
    u_long td_merges = 0; //current number of merges
    u_long td_saved = 0; //current number of merges
	...
	do {
		...
		while(syscall_cnt < td.size() && (!td[syscall_cnt].should_track || td[syscall_cnt].start_clock < clock )) { 
		    fprintf(stderr, "this is irregular...");
		    fprintf(stderr, "(%lu, %lu %lu %lu %lu), td: (%d %d, %lu %lu) %u\n", pid, clock, sysnum, td_merges, td_saved,
			    td[syscall_cnt].syscall, td[syscall_cnt].pid, td[syscall_cnt].start_clock, td[syscall_cnt].stop_clock, td[syscall_cnt].pin_traces.size());

		    td[syscall_cnt].num_merges = td[syscall_cnt-1].num_merges;
		    td[syscall_cnt].num_saved = td[syscall_cnt-1].num_saved;
	    ...
	    //update num_merges
	    td[syscall_cnt].num_merges = td_merges + num_merges; 
	    td[syscall_cnt].num_saved = td_saved + num_saved; 
...
static int map_next_file_part( struct pin_trace_iter &pti) { 
    ...
    fprintf(stderr,"%s mapped (0 %lx) of %lx\n", filename, pti.mapsize - 1, pti.filedone);
...
//	    Fprintf, "log mapping from (%lx %lx) of %lx\n",pti.fileoffset, pti.fileoffset + pti.mapsize - 1, pti.filedone);	    
	    ...
	    munmap((u_long*)pti.bufstart, pti.mapsize);
	    fprintf(stderr, "mapping from (%lx %lx) of %lx\n",pti.fileoffset, pti.fileoffset + pti.mapsize - 1, pti.filedone);	    
		...
		const unordered_set<pid_t> procs)
{
	...
	fprintf(stderr, "tracing through for pid %d\n",pid);
...
int parse_ulogs(vector<struct timing_data> &td, const char* dir)
{
	...
	if (!(i %100000)) fprintf(stderr, "looking at index %d\n",i);
...
int adjust_for_ckpts(vector<struct timing_data> &td,
	...
	if (t.start_clock >= ckpts[ckpt_index].rp_clock && 
	    ...
	    ckpt_index++;
	    ckpt_time = t.ftiming; 
	    fprintf(stderr, "%lu found the ckpt!, we'll be removing %lf time\n",t.start_clock,ckpt_time);
	}
	t.ftiming -= ckpt_time;
    }
}
